...
## USAGE: This script performs exploratory analyses of response variables in the database,
# plots density distribution and boxplots of each response variable by heated and control 
# groupings, with points colored by study id, then compares groups
...
setwd("~/Desktop/UBC/_Projects/doseResponse_analysis/writing/_PLoSOne_Submission/_4_RespVarStats/")
...
par(mfrow=c(1,2))
qqPlot(heat$resp.var) # samples are non-normal
qqPlot(control$resp.var) # samples appear close to normal
...
var.test(heat$resp.var, control$resp.var) # samples are of equal variances
